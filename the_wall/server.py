...
def wall_route():
    ...
    query2 = 'SELECT message_id, first_name, last_name, comment, DATE_FORMAT(comments.created_at, "%M %D %Y") AS date FROM comments JOIN messages ON messages.id = comments.message_id JOIN users ON users.id = comments.user_id ORDER BY comments.created_at'
